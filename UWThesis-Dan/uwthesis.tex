...
and testing out feedback control strategies for LPBF thermal processes. Along the course of formulating the framework, we explore control-oriented modeling of the LPBF process and advanced feedback control algorithms to mitigate in-process variations and improve part quality.
...
servo designs, such as RC design. Current inversion techniques have provided effective
...
Part \ref{part:Control-oriented Modeling} elaborates one essential component of the closed-loop simulation, that is, the finite element modeling. Besides, from underlying physics, we develop a new Hammerstein model that captures more of the complicated spatiotemporal thermomechanical dynamics.

In Part \ref{part:Multirate-Feedback-Control}, we investigate multirate feedback control algorithms, which serves as the other key component of the closed-loop simulation. In detail, Chapter \ref{chap:Loop-shaping-Control} introduces the general loop-shaping control methodologies. Chapter \ref{chap:Fractional-RC} builds a specific multirate fractional-order RC algorithm. Chapter \ref{chap:Model-Inversion} looks into the model inversion technique---an important piece of the loop-shaping control. Chapter \ref{chap:Spectral-Analysis} analyzes spectral properties of the closed-loop signals under multirate sample-data control.
...
real-time control framework \cite{Berumen:2010hq,frazier2014metal}. In hard disk drive systems, the sampling speed of the closed loop is limited by the amount of physical servo sectors
...
Under the infrastructure of the closed-loop simulation in Chapter \ref{chap:Closed-loop-Simulation}, we integrate control-oriented modeling in Part \ref{part:Control-oriented Modeling} with feedback control \ref{part:Multirate-Feedback-Control} and evaluate
...
the part quality in laser powder fusion (LPBF) additive manufacturing. Under the infrastructure of the closed-loop simulation, we investigate the control-oriented modeling and multirate feedback control, respectively. We draw the following conclusions from our study and present some potential avenues for future work.
...
Model inversion is fundamental to subsequent servo designs, such as RC design. We discuss new frequency-domain analysis and design
...
that the RC algorithm attenuates the periodic disturbances
